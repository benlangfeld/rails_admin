module RailsAdmin
  ...
  class MainController < RailsAdmin::ApplicationController
    ...
    def get_sort_hash
      ...
      elsif field.sortable.is_a?(String)
        field.sortable
    ...
    def check_for_injections(schema)
      ...
      allowed_associations = @model_config.list.visible_fields.select{ |f| f.association? && !f.association[:options][:polymorphic] }.map(&:association)
    ...
    def check_injections_for(model_config, methods_name)
      available_fields = model_config.list.visible_fields.select{ |f| !f.association? || f.association[:options][:polymorphic] }.map do |field|
